...
Note: Inside the sandbox environment, if you try to inspect these environment variables 
(e.g., echo $OPENCODE_API_KEY), they will literally print as proxy-managed.
This is expected behavior as the actual keys are managed by the sandbox proxy and never directly exposed to the guest microVM.
Usage
...
If you are running this from a different directory, provide the path to the
directory containing spec.yaml. Otherwise, you can leave the `--kit` flag out.
